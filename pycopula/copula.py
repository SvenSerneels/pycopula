...
class Copula():
    ...
    def _check_dimension(self, x):
        if len(x) != self.dim:
            raise ValueError(
                "Expected vector of dimension {0}, get vector of dimension {1}".format(self.dim, len(x)))
    ...
    def correlations(self, X):
        ...
        if self.dim != 2:
            raise Exception(
                "Correlations can not be computed when dimension is greater than 2.")
    ...
    def kendall(self):
        ...
        if self.kendall == None:
            raise ValueError(
                "You must compute correlations before accessing to Kendall's tau.")
        return self.kendall
    ...
    def pearson(self):
        ...
        if self.pearson == None:
            raise ValueError(
                "You must compute correlations before accessing to Pearson's r.")
        return self.pearson
    ...
    def spearman(self):
        ...
        if self.pearson == None:
            raise ValueError(
                "You must compute correlations before accessing to Spearman's rho.")
    ...
    def pdf(self, x):
        ...
        if self.name == 'indep':
            ...
            raise NotImplementedError(
                "PDF is not available for Fr̩chet-Hoeffding bounds.")
...
class ArchimedeanCopula(Copula):
    ...
    def pdf_param(self, x, theta):
        ...
        if invertNDerivative == None:
            try:
                invertNDerivative = lambda t, theta, order: scipy.misc.derivative(
                    lambda x: self.generatorInvert(x, theta), t, n=order, order=order+order % 2+1)
            except:
                raise Exception(
                    "The {0}-th derivative of the invert of the generator could not be computed.".format(self.dim))
        ...
        **kwargs
        ...
        # Canonical Maximum Likelihood Estimation
            ...
            def log_likelihood(theta):
                param_obs = np.apply_along_axis(
                    lambda x: self.pdf_param(x, theta), arr=pobs, axis=1)
        ...
        # Maximum Likelihood Estimation and Inference Functions for Margins
            ...
            if method == 'mle':
                res, estimationData = estimation.mle(self, X, marginals=kwargs.get('marginals', None), hyper_param=kwargs.get('hyper_param', None), hyper_param_start=kwargs.get('hyper_param_start', None), hyper_param_bounds=kwargs.get(
                    'hyper_param_bounds', None), theta_start=theta_start, theta_bounds=bounds, optimize_method=kwargs.get('optimize_method', 'Nelder-Mead'), bounded_optimize_method=kwargs.get('bounded_optimize_method', 'SLSQP'))
            else:
                res, estimationData = estimation.ifm(self, X, marginals=kwargs.get('marginals', None), hyper_param=kwargs.get('hyper_param', None), hyper_param_start=kwargs.get('hyper_param_start', None), hyper_param_bounds=kwargs.get(
                    'hyper_param_bounds', None), theta_start=theta_start, theta_bounds=bounds, optimize_method=kwargs.get('optimize_method', 'Nelder-Mead'), bounded_optimize_method=kwargs.get('bounded_optimize_method', 'SLSQP'))
...
class GaussianCopula(Copula):
    ...
    def __init__(self, dim=2, R=[[1, 0.5], [0.5, 1]],method='cmle',optimize_method='Nelder-Mead',bounded_optimize_method='SLSQP'):
        super(GaussianCopula, self).__init__(dim=dim)
        self.method = method
        self.optimize_method = optimize_method
        self.bounded_optimize_method = bounded_optimize_method
        self.set_corr(R)
    ...
    def set_corr(self, R):
        ...
        if len(S.shape) > 2:
            raise ValueError(
                "2-dimensional array expected, get {0}-dimensional array.".format(len(S.shape)))
        if S.shape[0] != S.shape[1]:
            raise ValueError(
                "Correlation matrix must be a squared matrix of dimension {0}".format(self.dim))
        if ((S - S.T) > 1e-10).any():
    ...
    def pdf_param(self, x, R):
        ...
        if len(np.asarray(R).shape) == 2 and len(R) != self.dim:
            raise ValueError(
                "Expected covariance matrix of dimension {0}.".format(self.dim))
        u = norm.ppf(x)
    ...
    def fit(self, X, method='cmle', verbose=True, **kwargs):
        ...
        **kwargs
        ...
        tril_indices = np.tril_indices(self.dim, -1)
        triu_indices = (tril_indices[1],tril_indices[0])
...
#                        S[self.dim - i - 1][self.dim - j - 1] = S[i][j
                ...
                lh = 0
                for i in range(n):
                    cDens = RDet**(-0.5) * np.exp(-0.5 *
                                   np.dot(ICDF[i, :], np.dot(RInv, ICDF[i, :])))
                    lh += np.log(cDens)
            ...
            res = estimation.cmle(log_likelihood,
                ...
                bounded_optimize_method=self.bounded_optimize_method)
            rho = res['x']
        elif method == 'mle':
            rho_start = [ 0.0 for i in range(int(self.dim * (self.dim - 1) / 2)) ]
            res, estimationData = estimation.mle(self, X, marginals=kwargs.get('marginals', None), hyper_param=kwargs.get('hyper_param', None), hyper_param_start=kwargs.get('hyper_param_start', None), hyper_param_bounds=kwargs.get('hyper_param_bounds', None), theta_start=rho_start, optimize_method=self.optimize_method, bounded_optimize_method=self.bounded_optimize_method)
            rho = res['x']
            
        self.R = np.identity(self.dim)
        # We extract rho values to covariance matrix
        self.R[tril_indices] = rho
        self.R[triu_indices] = rho
...
class StudentCopula(Copula):
    ...
    def __init__(self, dim=2, df=1, R=[[1, 0], [0, 1]],
                 method='cmle',optimize_method='Nelder-Mead',
                 bounded_optimize_method='SLSQP'):
